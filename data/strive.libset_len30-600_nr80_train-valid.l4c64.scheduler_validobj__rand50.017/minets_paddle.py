...
def stack_linear_block(channel_sizes, data_fmt=None, pre_act_norm=False, act_after_norm=False,
    ...
    if resnet and not is_return:
        ...
        block_layers.append(MyResNet(beta=resnet_beta))
...
class Seq2MatCastxD_OLD(nn.Layer):
    def __init__(self, method='concat', in_fmt='NCL', out_fmt='NCHW',
            in_channels=None, in_linear=False,
            out_channels=None, out_norm='layer', out_act='swish'):
        super(Seq2MatCastxD_OLD, self).__init__()
        ...
        dim_map = dict(CONCAT=2, CONCATENATE=2, ADD=1, MULTIPLY=1)
        self.mul_channels = dim_map.get(self.method, 1)

        if self.in_linear:
            assert self.in_channels, "in_channels must be specified for self.pre_linear=True"
            self.InLinear1 = nn.Linear(self.in_channels, self.in_channels)
            self.InLinear2 = nn.Linear(self.in_channels, self.in_channels)

        if self.out_channels:
            assert self.in_channels, "in_channels must be specified for self.out_channels=True"
            self.PostSeq2Mat = nn.Sequential(
                nn.Linear(self.in_channels * self.mul_channels, self.out_channels, bias_attr=False),
        ...
        logger.info(f'        in_linear: {self.in_linear}')
    ...
    def forward(self, xh, xw):
        ...
        if self.in_linear:
            xh = self.InLinear1(xh)
            xw = self.InLinear2(xw)

        N, CH, H = xh.shape
        N1, CW, W = xw.shape
        assert N == N1, f"Two matrices must have the same N: {N} != {N1}!"

        xh = xh.unsqueeze(3).expand((N, CH, H, W))
        xw = xw.unsqueeze(2).expand((N, CW, H, W))

        if self.method.startswith('CONCAT'):
            x = mi.concat([xh, xw], axis=1) # --> [N, C+C1, L, L]
        elif self.method.startswith('ADD'):
            assert CH == CW, f"Cannot add two matrices with different C: {CH} != {CW}"
            x = xh + xw
        elif self.method.startswith('MULTIPLY'):
            assert CH == CW, f"Cannot multiply two matrices with different C: {CH} != {CW}"
            x = xh * xw
...
class Seq2MatCastxD(nn.Layer):
    def __init__(self, method='concat', in_fmt='NCL', out_fmt='NCHW',
        ...
        if self.in_linear:
            assert self.in_channels, "in_channels must be specified for self.in_linear=True"
            self.InLinear1 = nn.Linear(self.in_channels, self.in_channels)
            self.InLinear2 = nn.Linear(self.in_channels, self.in_channels)
    ...
    def forward(self, xh, xw):
        """ xh: [NHC], xw: [NWC], H and W can be different """
        if self.in_fmt == 'NLC':
            pass
        elif self.in_fmt == 'NCL':
            xh = xh.transpose([0, 2, 1])
            xw = xw.transpose([0, 2, 1])
        ...
        NH, H, CH,= xh.shape
        NW, W, CW = xw.shape
        assert NH == NW, f"Two matrices must have the same N: {NH} != {NW}!"

        xh = xh.unsqueeze(2).expand((NH, H, W, CH))
        xw = xw.unsqueeze(1).expand((NW, H, W, CW))

        if self.method.startswith('CONCAT'):
            x = mi.concat([xh, xw], axis=-1) # --> [N, H, W, CH+CW]
        elif self.method.startswith('ADD'):
            assert CH == CW, f"Cannot add two matrices with different C: {CH} != {CW}"
            x = xh + xw
        elif self.method.startswith('MULTIPLY'):
        ...
        if self.out_fmt == 'NHWC':
            return x
        elif self.out_fmt == 'NWHC':
            return x.transpose([0, 2, 1, 3])
        elif self.out_fmt == 'NCHW':
            return x.transpose([0, 3, 1, 2])
        elif self.out_fmt == 'NCWH':
...
class MyEmbdxDLayer(nn.Layer):
    ...
    def __init__(self, args, in_channels=None, in_dtype=None, out_dtype=None, padding_idx=0,
        ...
        # get default input_shapes for mi.summary() calls
    ...
    def forward(self, x, seqs_len):
        ...
        for OneLayer in self.LayerList:
            if isinstance(OneLayer, nn.Embedding): # not necessary yet ...
                x = OneLayer(x)
            elif isinstance(OneLayer, MyNormxD): # accepts seqs_len
...
class MyLinearTower(nn.Layer):
    ...
    def forward(self, x, seqs_len):
        ...
        for OneLayer in self.LayerList:
            if isinstance(OneLayer, MyResNet): # self.linear_resnet
                x = x0 + OneLayer(x)
                x0 = x
            elif isinstance(OneLayer, MyNormxD): # accepts seqs_len
                x = OneLayer(x, seqs_len)
    ...
    def forward(self, x, seqs_len):
        ...
        for OneLayer in self.LayerList:
            if isinstance(OneLayer, nn.LSTM):
                ...
                initial_states = (h, c)
            elif isinstance(OneLayer, MyResNet):
                x = x0 + OneLayer(x)
                x0 = x
            elif isinstance(OneLayer, MyNormxD):
...
class MyConv1DTower(nn.Layer):
    def __init__(self, args, in_channels=None, data_fmt=None, is_return=False):
        ...
        fn_fix_length = lambda x: misc.fix_length1d(x, len(self.channel_sizes), constant_values=x[-1])
        ...
        if args.conv1d_padding is None:
            self.padding = mitas_utils.calc_conv_padding(self.kernel_size, stride=self.stride,
    ...
    def forward(self, x, seqs_len):
        ...
        for OneLayer in self.LayerList:
            if  isinstance(OneLayer, MyResNet):
                x = x0 + OneLayer(x)
                x0 = x
            elif isinstance(OneLayer, MyNormxD):
                x = OneLayer(x, seqs_len)
...
class MyAttnxDTower(nn.Layer):
    ...
    def forward(self, q, k=None, v=None, seqs_len=None):
        ...
        q0_shape = q.shape
        if self.norm_in is not None:
            if isinstance(self.NormIn, MyNormxD):
        ...
        for i, OneLayer in enumerate(self.LayerList):
            ...
            elif isinstance(OneLayer, MyNormxD): # AxialAttention doesnot support mask or batch_size > 1
...
def MyReturnTower(args, in_channels=None, label_dim=None, data_fmt=None, **kwargs):
    new_args = misc.Struct(vars(args)).update(
                linear_dim = args.return_dim + ([label_dim] if label_dim else []),
...
class Seq2Seq_Conv1DNet(nn.Layer):
    ...
    def __init__(self, args):
        ...
        in_channels = self.in_channels # the running channel dim
        ...
        self.OutLinear = MyReturnTower(args, label_dim=args.label_dim[0],
            in_channels=in_channels, data_fmt='NLC')
...
class Seq2Mat_Conv2DNet(nn.Layer):
    ...
    def __init__(self, args):
        ...
        out_channels2d = args.conv2d_dim[0]
        self.Seq2Mat = Seq2MatCastxD(method=self.seq2mat_method, in_fmt='NLC', out_fmt='NHWC',
            in_linear=True, in_channels=in_channels, out_channels=out_channels2d)
...
class Seq2Mat_LSTMNet(nn.Layer):
    ...
    def __init__(self, args):
        ...
        out_channels = args.return_dim[0]
        self.Seq2Mat = Seq2MatCastxD(method=self.seq2mat_method, in_fmt='NLC', out_fmt='NHWC',
            in_linear=True, in_channels=in_channels, out_channels=out_channels)
        in_channels = out_channels

        self.OutLinear = MyReturnTower(args, in_channels=in_channels,
            label_dim=args.label_dim[0], data_fmt='NHWC')
...
class Seq2MatSeq_Attn1DConv2DConv1D_ContarnaNet(nn.Layer):
    """ """
    def __init__(self, args):
        super(Seq2MatSeq_Attn1DConv2DConv1D_ContarnaNet, self).__init__()
...
class Seq2Mat_LSTMConv2DNet(nn.Layer):
    """ This network ignores all inter-residue information  """
    def __init__(self, args):
        ...
        self.OutLinear = MyReturnTower(args, in_channels=in_channels2d,
            label_dim=args.label_dim[0], data_fmt='NHWC')
...
class Seq2MatNum_LSTMConv2DLSTMNet(nn.Layer):
    ...
    def __init__(self, args):
        ...
        f1lstm_num = min([args.lstm_num, 1])
        f1lstm_dim = min(args.lstm_dim + [32])

        self.F1LSTMh = MyLSTM1DTower(args, in_channels=in_channels2d, data_fmt='NLC',
            lstm_num=f1lstm_num, lstm_dim=f1lstm_dim, train_initial=False, return_states=True)
        in_channels = self.F1LSTMh.out_channels

        self.F1LSTMw = MyLSTM1DTower(args, in_channels=in_channels, data_fmt='NLC',
            lstm_num=f1lstm_num, lstm_dim=f1lstm_dim, train_initial=False, return_states=True)
        in_channels = self.F1LSTMw.out_channels

        self.F1OutLinear = MyReturnTower(args, in_channels=in_channels, data_fmt='NLC',
            linear_dim=[f1lstm_dim] * 2 + [args.label_dim[1]], linear_num=1, is_return=True)
    ...
    def forward(self, x, seqs_len=None):
        ...
        # F1 prediction
        x = x2d.reshape([-1, x2d.shape[-2], x2d.shape[-1]]).detach()
        _, (_, x) = self.F1LSTMh(x, seqs_len=mi.tile(seqs_len, x2d.shape[1:-2]))

        # x=c: [num_layer*num_direction, batch_size, hidden_size]
        x = mi.reshape(mi.transpose(x, perm=[1, 2, 0]), [x.shape[1], -1])
        x = mi.reshape(x, [batch_size, -1, x.shape[-1]])

        _, (_, x) = self.F1LSTMw(x, seqs_len=seqs_len)
        # x=c: [num_layer*num_direction, batch_size, hidden_size]
        x = mi.reshape(mi.transpose(x, perm=[1, 2, 0]), [x.shape[1], -1])
        f1 = F.sigmoid(self.F1OutLinear(x, seqs_len=seqs_len))
        # if f1.shape[-1] == 1:
            # f1 = mi.squeeze(f1, -1)
...
class Seq2MatNum_LSTMConv2DLSTM2Net(nn.Layer):
    """ """
    def __init__(self, args):
        super(Seq2MatNum_LSTMConv2DLSTM2Net, self).__init__()
        ...
        # the original input is appended to the 2D matrix
        self.F1LSTMh = MyLSTM1DTower(args, in_channels=in_channels2d+24, data_fmt='NLC',
            lstm_num=f1lstm_num, lstm_dim=f1lstm_dim, train_initial=False, return_states=True)
        in_channels = self.F1LSTMh.out_channels
    ...
    def forward(self, x, seqs_len=None):
        ...
        # F1 prediction
        # add the original input first

        N, L, C = x0.shape
        x = mi.concat([x2d.detach(), x0.unsqueeze(1).expand((N, L, L, C)), x0.unsqueeze(2).expand((N, L, L, C))], axis=-1)
...
class Seq2MatSeq_LSTMConv2DConv1D_DistarnaNet(nn.Layer):
    """ """
    def __init__(self, args):
        super(Seq2MatSeq_LSTMConv2DConv1D_DistarnaNet, self).__init__()
        ...
        self.SeqEmbed = MyEmbdxDLayer(args, in_channels=in_channels, out_fmt='NLC')
        in_channels = self.SeqEmbed.out_channels

        self.SeqInitLinear = MyInitTower(args, in_channels=in_channels, data_fmt='NLC')
        in_channels = self.SeqInitLinear.out_channels

        self.Seq2SeqLSTM = MyLSTM1DTower(args, in_channels=in_channels, data_fmt='NLC')
        in_channels = self.Seq2SeqLSTM.out_channels
        ...
        self.MatConv2D = MyConv2DTower(args, in_channels=in_channels2d, data_fmt='NHWC')
        in_channels2d = self.MatConv2D.out_channels

        # linear_dim2d = [in_channels2d] * 3
        self.MatOutLinear = MyReturnTower(args, in_channels=in_channels2d, label_dim=args.label_dim[0],
            data_fmt='NHWC')

        # add Conv1D towers to predict Seq (the torsion angles for now)
        if self.label_num > 1:
            lstm2seq_num = min([args.lstm_num, 1])
            lstm2seq_dim = min(args.lstm_dim + [32])

            self.Mat2SeqLSTM = MyLSTM1DTower(args, in_channels=in_channels2d, data_fmt='NLC',
                lstm_num=lstm2seq_num, lstm_dim=lstm2seq_dim, train_initial=False, return_states=True)
            in_channels = self.Mat2SeqLSTM.out_channels

            out_channels1d = args.conv1d_dim[0]
            self.SeqPreConv1D = nn.Sequential(
            ...
            self.SeqConv1D = MyConv1DTower(args, in_channels=in_channels, data_fmt='NLC')
            in_channels = self.SeqConv1D.out_channels

            # linear_dim1d = [in_channels] * 3
            self.SeqOutLinear = MyReturnTower(args, in_channels=in_channels, label_dim=args.label_dim[1],
                data_fmt='NLC')
    ...
    def forward(self, x, seqs_len=None):
        ...
        x, seqs_len = self.SeqEmbed(x, seqs_len=seqs_len)
        x = self.SeqInitLinear(x, seqs_len=seqs_len)
        x = self.Seq2SeqLSTM(x, seqs_len=seqs_len)

        x2d = self.Seq2Mat(x, x)

        x2d = self.MatConv2D(x2d, seqs_len=seqs_len)
        x2d = (x2d + mi.transpose(x2d, perm=[0, 2, 1, 3])) / 2.0

        y = [self.MatOutLinear(x2d, seqs_len=seqs_len)]

        if self.label_num > 1:
            x = x2d.reshape([-1, x2d.shape[-2], x2d.shape[-1]])
            _, (_, x) = self.Mat2SeqLSTM(x, seqs_len=mi.tile(seqs_len, x2d.shape[1:-2]))
            # x=c: [num_layer*num_direction, batch_size, hidden_size]
            x = mi.reshape(mi.transpose(x, perm=[1, 2, 0]), [x.shape[1], -1])
            x = mi.reshape(x, [batch_size, -1, x.shape[-1]])

            x = self.SeqPreConv1D(x)
            x = self.SeqConv1D(x, seqs_len=seqs_len)
            x = self.SeqOutLinear(x, seqs_len=seqs_len)

            y.append(x)

        # if y[0].shape[-1] == 1:
        #     y[0] = mi.squeeze(y[0], -1)
        return y

    def summary(self, input_size=None):
        if input_size is None:
            input_size = self.SeqEmbed.in_shapes
        return mi.summary(self, input_size)
